...
$tick = array(
    ...
    'inprogress' => 0,
    'waiting' => 0,
/*  'closed' => 0, Removed closed items from Pie */
    'deleted' => 0,
);
$total = 0;
...
if ($result) {
    while ($objp = $db->fetch_object($result)) {
       ...
       /* if ($objp->fk_statut == 8) {
            $tick['closed'] = $objp->nb; 
        }  Removed closed items from Pie */
        if ($objp->fk_statut == 9) {
            $tick['deleted'] = $objp->nb;
        }
    }

    if ((round($tick['unread']) ? 1 : 0) + (round($tick['read']) ? 1 : 0) + (round($tick['answered']) ? 1 : 0) + (round($tick['assigned']) ? 1 : 0) + (round($tick['inprogress']) ? 1 : 0) + (round($tick['waiting']) ? 1 : 0) >= 2) {
        $dataseries = array();
...
/*      $dataseries[] = array('label' => $langs->trans("Closed"), 'data' => round($tick['Closed']));  Removed closed items from Pie */
        $dataseries[] = array('label' => $langs->trans("Deleted"), 'data' => round($tick['closed']));
    }
} else {
    dol_print_error($db);
}
